<template>
  <header
    :class="[
      $options.name,
      'border__bottom color__bg--contrast color__border--base--ghost padding__left--m padding__right--m',
    ]"
  >
    <section :class="$options.name + '__logo'">
      <nuxt-link
        to="/"
        active-class
        exact-active-class
...
.Header {
  ...
  &__logo {
    ...
    a {
      ...
      display: inline-flex;
      height: calc(var(--size__m) + var(--size__s));
      justify-content: center;
      @include breakpoint(m) {
        height: var(--size__l);
      }
